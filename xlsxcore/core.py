...
from xlsxcore.compat import OPENPYXL_24
from xlsxcore.format import format_cell
...
def get_styles_from_cell(cell, merged_cell_map=None):
    ...
    if cell.font:
        h_styles['font-size'] = '12px'  # "%spx" % cell.font.sz
        if cell.font.color:
            h_styles['color'] = normalize_color(cell.font.color)
            if h_styles['color'] == '#FF0000':
                h_styles['font-size'] = '14px'
        if cell.font.b:
        ...
        if cell.font.strike:
            h_styles['text-decoration'] = 'line-through'
...
def worksheet_to_data(ws, locale=None, fs=None):
    ...
    for row_i, row in enumerate(ws.iter_rows()):
        ...
        for col_i, cell in enumerate(row):
            ...
            if cell.value is "0" or cell.value is 0:
                cell.value = " 0 "
